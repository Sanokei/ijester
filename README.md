...
> **Repository status:** implemented (MVP+). The scaffold described below is built and tested: iris UI (canvas-rendered blue pupil + orange waveform ring), consent flow, per-visit Durable Objects, continuous streamed 16 kHz PCM over WebSocket with server-side pause-aware windowing, Workers AI (Whisper) transcription with a mock fallback, the DeepSeek `deepseek-v4-flash` classifier with a heuristic fallback, the deterministic policy gate, 20 synthesized CC0 sounds, an activity feed, and unit + Workers-runtime tests. (The client streams raw PCM rather than MediaRecorder blobs — Whisper rejects header-less webm chunks; the server wraps flushed windows as WAV.) See [`SPEC.md`](./SPEC.md) for the complete product, safety, privacy, and architecture requirements.
>
